...
if not isinstance(lib, ErrorHandlerMockBNBNativeLibrary):
    ...
    def _gemm_8bit_cpu(
        ...
        A_f = A.reshape(-1, A.shape[-1]).contiguous()
        B_c = B.contiguous()
        ...
        lib.cgemm_8bit_inference_cpu_fp32(
            ...
            ct.c_longlong(N),
            ct.c_longlong(K),
            ct.c_longlong(A_f.stride(0)),
            ct.c_longlong(B_c.stride(0)),
            ct.c_longlong(out.stride(0)),
            ct.c_longlong(blocksize),
        )
...
def _optimizer_update_8bit_blockwise_cpu(
    ...
    # Fused single-pass C kernel (dequant + update + p update + requant in one
    # sweep). Falls back to the composite PyTorch path below when the native
    # library is unavailable or the optimizer is not supported by the kernel.
    _OPT_IDS = {"adam": 0, "lamb": 0, "momentum": 1, "lars": 1, "lion": 2, "rmsprop": 3, "adagrad": 4, "ademamix": 5}
    n = p.numel()
    # The C kernel walks raw pointers and assumes C-contiguous layouts: a
    # non-contiguous grad (transposed / sliced views are common in training)
    # would silently read the wrong elements and corrupt the states.
    contiguous_layout = (
        g.is_contiguous()
        and p.is_contiguous()
        and state1.is_contiguous()
        and (state2 is None or state2.is_contiguous())
        and qmap1.is_contiguous()
        and (qmap2 is None or qmap2.is_contiguous())
        and absmax1.is_contiguous()
        and (absmax2 is None or absmax2.is_contiguous())
    )
    if (
        not isinstance(lib, ErrorHandlerMockBNBNativeLibrary)
        and contiguous_layout
        and optimizer_name in _OPT_IDS
        and g.dtype in (torch.float32, torch.bfloat16, torch.float16)
        # ademamix packs a second momentum into state1[n:2n] with its absmax in
        # absmax1[blocks:2*blocks]; the flat C layout requires that packing.
        and (optimizer_name != "ademamix" or state1.numel() >= 2 * n)
    ):
        try:
            dtype_id = {torch.float32: 0, torch.bfloat16: 1, torch.float16: 2}[g.dtype]
            # Every scalar needs an explicit ctypes width: a bare Python int is
            # marshalled as 32-bit c_int, but `n` is a long long. libffi writes
            # only the low 4 bytes of its 8-byte stack slot; the C side reads
            # all 8, so stale stack bits leak into n (observed: 0x7FFF00004000
            # for n=16384) and the kernel walks out of bounds -> SIGSEGV.
            lib.coptimizer_update_8bit_blockwise_cpu(
                ct.c_int(_OPT_IDS[optimizer_name]),
                get_ptr(g),
                get_ptr(p),
                get_ptr(state1),
                get_ptr(state2) if state2 is not None else ct.c_void_p(0),
                ct.c_float(beta1),
                ct.c_float(beta2),
                ct.c_float(beta3),
                ct.c_float(alpha),
                ...
                ct.c_float(lr),
                get_ptr(qmap1),
                get_ptr(qmap2) if qmap2 is not None else ct.c_void_p(0),
                get_ptr(absmax1),
                get_ptr(absmax2) if absmax2 is not None else ct.c_void_p(0),
                ct.c_float(weight_decay),
                ct.c_float(gnorm_scale),
                ct.c_int(1 if skip_zeros else 0),
                ct.c_longlong(n),
                ct.c_int(dtype_id),
            )
            return
        except (AttributeError, RuntimeError):
            pass  # old native library without the fused entry point
