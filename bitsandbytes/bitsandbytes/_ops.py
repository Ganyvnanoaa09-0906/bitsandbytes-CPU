...
torch.library.define(
    "bitsandbytes::gemm_8bit",
    "(Tensor A, Tensor B, Tensor absmax, int blocksize) -> Tensor",
)


@register_fake("bitsandbytes::gemm_8bit")
def _(A: torch.Tensor, B: torch.Tensor, absmax: torch.Tensor, blocksize: int) -> torch.Tensor:
    torch._check(blocksize > 0, lambda: f"blocksize must be positive, got {blocksize}")
    ...
    torch._check(absmax.dtype == torch.float32, lambda: f"absmax must be float32, got {absmax.dtype}")
    N = B.shape[0]
    return torch.empty((*A.shape[:-1], N), device=A.device, dtype=A.dtype)
